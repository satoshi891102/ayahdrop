...
// Popular/commonly shared ayahs for quick picks
export const POPULAR_AYAHS = [
  { surah: 2, ayah: 255, label: "Ayat al-Kursi" },
  { surah: 2, ayah: 286, label: "Last ayah of Al-Baqarah" },
  { surah: 3, ayah: 173, label: "Hasbunallahu wa ni'mal wakeel" },
  { surah: 13, ayah: 28, label: "Verily in the remembrance of Allah..." },
  { surah: 2, ayah: 152, label: "Remember Me, I will remember you" },
  { surah: 94, ayah: 5, label: "With hardship comes ease" },
  { surah: 94, ayah: 6, label: "With hardship comes ease (repeated)" },
  ...
  { surah: 9, ayah: 51, label: "Nothing will happen to us..." },
  { surah: 29, ayah: 69, label: "Those who strive for Us..." },
  { surah: 55, ayah: 13, label: "Which favors will you deny?" },
  { surah: 112, ayah: 1, label: "Say: He is Allah, the One" },
  { surah: 1, ayah: 1, label: "Bismillah - Al-Fatihah" },
  { surah: 36, ayah: 58, label: "Peace — a word from a Merciful Lord" },
  { surah: 93, ayah: 5, label: "Your Lord will give you..." },
  { surah: 2, ayah: 45, label: "Seek help through patience and prayer" },
  { surah: 49, ayah: 13, label: "Most noble is the most righteous" },
...
export async function fetchAyah(surah: number, ayah: number): Promise<AyahData> {
  ...
  if (json.code !== 200 || !json.data || json.data.length < 2) {
    throw new Error(`Invalid response for ayah ${surah}:${ayah}`);
  }
  ...
  return {
    ...
    surahNumber: surah,
    ayahNumber: ayah,
    reference: `Surah ${arabicData.surah.englishName} (${surah}:${ayah})`,
  };
}
